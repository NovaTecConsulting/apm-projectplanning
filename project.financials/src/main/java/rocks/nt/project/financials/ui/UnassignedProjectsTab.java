...
public class UnassignedProjectsTab {
	...
	private void createProjectsTab(VerticalLayout projectsTab) {
		final ActiveSpan s_thisSpan = JaegerUtil.getInstance().createNewActiveSpan("Open Unassigned Projects Tab");
		try {
			// Form Layout
	...
	private void createUnassignedProject() {
		final ActiveSpan s_thisSpan = JaegerUtil.getInstance().createNewActiveSpan("Create Unassigned Project");
		s_thisSpan.setTag(JaegerUtil.T_PROJECT, projectComboBox.getValue());
		s_thisSpan.setTag(JaegerUtil.T_FROM, fromDateField.getValue().toString());
		s_thisSpan.setTag(JaegerUtil.T_TO, toDateField.getValue().toString());
		...
		try {
			...
			InfluxService.getInstance().createUnassignedProject(project, fromDateField.getValue(),
					toDateField.getValue(), notesField.getValue().trim(), color);
			...
			s_thisSpan.log("Error: " + throwable.getMessage());
		} finally {
			s_thisSpan.deactivate();
		}
	}
	...
	private void deleteUnassignedProject() {
		final ActiveSpan s_thisSpan = JaegerUtil.getInstance().createNewActiveSpan("Delete Unassigned Project");
		s_thisSpan.setTag(JaegerUtil.T_PROJECT, projectComboBox.getValue());

		try {
			String project = projectComboBox.getValue();

			InfluxService.getInstance().deleteUnassignedProject(project, fromDateField.getValue(),
